...
def recover_password(
    ...
    set_password_reset_token(db, user.id, token, expires_at)
...
def reset_password_endpoint(
    ...
    """
    user = get_user_by_reset_token(db, token=request.token)
    if not user:
        raise HTTPException(status_code=400, detail="Invalid or expired recovery token")
    
    reset_password(db, user_id=user.id, new_password=request.new_password)
