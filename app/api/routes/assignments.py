...
def api_get_assignment_history(
    ...
    return get_assignment_history(db, type, limit)

@router.post("/{type}/generate", response_model=List[Dict[str, Any]])
def api_generate_assignments(
    type: str,
    ...
    assignments = generate_weekly_assignments(db, type, n_weeks, start_date)
    return [
