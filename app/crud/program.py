...
def _sync_assignment_history(db: Session, program_id: int, week_start: date, payload: dict):
    # Borrar historial viejo
    db.execute(delete(AssignmentHistory).where(AssignmentHistory.program_id == program_id))
...
def get_current_program_id(db: Session, today: date) -> int:
    stmt = (
...
def get_program(db: Session, prog_id: int):
...
def get_staging_programs(db: Session):
    stmt = (
        select(
...
def update_staging_program(db: Session, prog_id: int, program: ProgramUpdate):
    stmt = (
...
def delete_staging_program(db: Session, prog_id: int):
    stmt = delete(MeetingProgramStaging).where(MeetingProgramStaging.id == prog_id)
    result = db.execute(stmt)
    db.commit()
    return result.rowcount > 0

def update_program(db: Session, prog_id: int, program: ProgramUpdate):
    stmt = (
    ...
    _sync_assignment_history(db, prog_id, program.week_start, program.payload)
    
    db.commit()
    return result.scalar_one_or_none()

def delete_program(db: Session, prog_id: int):
    stmt = delete(MeetingProgram).where(MeetingProgram.id == prog_id)
    result = db.execute(stmt)
    db.commit()
    return result.rowcount > 0

def publish_program(db: Session, prog_id: int):
    # 1. Get from staging
    staging_prog = get_staging_program(db, prog_id)
    if not staging_prog:
        return None
    ...
    # 4. Sync history
    _sync_assignment_history(db, next_id, staging_prog.week_start, staging_prog.payload)
